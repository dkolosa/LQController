...
def main():
    ...
    e0 = 0.1
    i0 = math.radians(1.0)
    Omega0 = math.radians(20.0)
    w0 = math.radians(15.0)
    M0 = math.radians(15.0)
...
def find_G_M(a, e, i, w):
    ...
    G[3, 10] = 0.5*(1+e**2)*math.sin(w)  # a1W
    G[3, 11] = -0.25*e*math.sin(w)  # a2W
    G[3, 12] = 0.5*math.sqrt(1-e**2)*math.cos(w)  # b1W
    G[3, 13] = -0.25*e*math.sqrt(1-e**2)*math.cos(w)  # b2W
    ...
    G[5, :] = G[5,:]+(1 - math.sqrt(1-e**2)) * (G[4,:]+G[3,:])+\
              2*math.sqrt(1-e**2)*(math.sin(i/2))**2*G[3,:]-(G[4,:]+G[3,:])
